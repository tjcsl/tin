...
def serialize_submission_info(submission, user) -> Dict[str, Union[None, int, float, str, bool]]:
    if (
        submission.assignment.course in user.courses.all()
        or user == submission.assignment.course.teacher
        ...
        data = {
            ...
            "points_received": (
                float(submission.points_received)
                if submission.points_received is not None
                else None
            ),
            "points_possible": (
                float(submission.points_possible)
                if submission.points_possible is not None
                else None
            ),
